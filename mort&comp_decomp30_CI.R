...
for (i in 1:1000) {
  ...
  Mx_tot_m <- 
    mean(PropN_BH_m[1:15])*Mx_BH_m + mean(PropN_PH_m[1:15])*Mx_PH_m + 
    mean(PropN_PNH_m[1:15])*Mx_PNH_m + mean(PropN_NPH_m[1:15])*Mx_NPH_m +
    mean(PropN_NPNH_m[1:15])*Mx_NPNH_m
  Mx_tot_f <- 
    mean(PropN_BH_f[1:15])*Mx_BH_f + mean(PropN_PH_f[1:15])*Mx_PH_f + 
    mean(PropN_PNH_f[1:15])*Mx_PNH_f + mean(PropN_NPH_f[1:15])*Mx_NPH_f +
    mean(PropN_NPNH_f[1:15])*Mx_NPNH_f
  ...
  dCx_BH <- rep(dydx_decomp(mean(PropN_BH_m[1:15]), mean(PropN_BH_f[1:15])),length(lx_avg))
  dCx_PH <- rep(dydx_decomp(mean(PropN_PH_m[1:15]), mean(PropN_PH_f[1:15])),length(lx_avg))
  dCx_PNH <- rep(dydx_decomp(mean(PropN_PNH_m[1:15]), mean(PropN_PNH_f[1:15])),length(lx_avg))
  dCx_NPH <- rep(dydx_decomp(mean(PropN_NPH_m[1:15]), mean(PropN_NPH_f[1:15])),length(lx_avg))
  dCx_NPNH <- rep(dydx_decomp(mean(PropN_NPNH_m[1:15]), mean(PropN_NPNH_f[1:15])),length(lx_avg))
  ...
  mCx_BH <- (rep(mean(PropN_BH_m[1:15]),length(lx_avg)) + rep(mean(PropN_BH_f[1:15]),length(lx_avg)))/2
  mCx_PH <- (rep(mean(PropN_PH_m[1:15]),length(lx_avg)) + rep(mean(PropN_PH_f[1:15]),length(lx_avg)))/2
  mCx_PNH <- (rep(mean(PropN_PNH_m[1:15]),length(lx_avg)) + rep(mean(PropN_PNH_f[1:15]),length(lx_avg)))/2
  mCx_NPH <- (rep(mean(PropN_NPH_m[1:15]),length(lx_avg)) + rep(mean(PropN_NPH_f[1:15]),length(lx_avg)))/2
  mCx_NPNH <- (rep(mean(PropN_NPNH_m[1:15]),length(lx_avg)) + rep(mean(PropN_NPNH_f[1:15]),length(lx_avg)))/2
...
data.table::data.table(
  gap = c(round(mean.gap2,2), round(CI.gap2[1],2), round(CI.gap2[2],2)),
  mortality = c(round(mean.mort,2), round(CI.mort[1],2), round(CI.mort[2],2)),
  composition = c(round(mean.comp,2), round(CI.comp[1],2), round(CI.comp[2],2))
)
...
ggplot(age_con, aes(edu,cont, fill = age))+
  ...
  labs(subtitle = "Age-group & Education levels contribution, Hypothetical scenario",
       x = "Education levels", y = "Contribution")+
  guides(fill = guide_legend("Age group"))+
  theme_bw()
ggsave("reports/PNAS-age30.pdf")
