...
###
### life expectancy sex gap at age 60
###
...
for (i in 1:1000) {
  ...
  # lx_male <- cumprod(lifetable(25:104,Nx_TOT_M,death.male,
  #                      sex = 'M')$px)
  # lx_female <- cumprod(lifetable(25:104,Nx_TOT_F,death.female,
  #                      sex = 'F')$px)
  # lx_avg <- (lx_male+lx_female)/2
  
  lx_male <- cumprod(px_func(Mx_tot_m[36:80]))
  lx_female <- cumprod(px_func(Mx_tot_f[36:80]))
  lx_avg <- (lx_male + lx_female)/2
  
  gap <- c(gap, sum(lx_female)/lx_female[1] - sum(lx_male)/lx_male[1])
  ...
  dMx_BH <- dydx_decomp(Mx_BH_m[36:80], Mx_BH_f[36:80])
  dMx_PH <- dydx_decomp(Mx_PH_m[36:80], Mx_PH_f[36:80])
  dMx_PNH <- dydx_decomp(Mx_PNH_m[36:80], Mx_PNH_f[36:80])
  dMx_NPH <- dydx_decomp(Mx_NPH_m[36:80], Mx_NPH_f[36:80])
  dMx_NPNH <- dydx_decomp(Mx_NPNH_m[36:80], Mx_NPNH_f[36:80])
  
  dCx_BH <- dydx_decomp(PropN_BH_m[36:80], PropN_BH_f[36:80])
  dCx_PH <- dydx_decomp(PropN_PH_m[36:80], PropN_PH_f[36:80])
  dCx_PNH <- dydx_decomp(PropN_PNH_m[36:80], PropN_PNH_f[36:80])
  dCx_NPH <- dydx_decomp(PropN_NPH_m[36:80], PropN_NPH_f[36:80])
  dCx_NPNH <- dydx_decomp(PropN_NPNH_m[36:80], PropN_NPNH_f[36:80])
  
  mMx_BH <- (Mx_BH_m[36:80] + Mx_BH_f[36:80])/2
  mMx_PH <- (Mx_PH_m[36:80] + Mx_PH_f[36:80])/2
  mMx_PNH <- (Mx_PNH_m[36:80] + Mx_PNH_f[36:80])/2
  mMx_NPH <- (Mx_NPH_m[36:80] + Mx_NPH_f[36:80])/2
  mMx_NPNH <- (Mx_NPNH_m[36:80] + Mx_NPNH_f[36:80])/2
  
  mCx_BH <- (PropN_BH_m[36:80] + PropN_BH_f[36:80])/2
  mCx_PH <- (PropN_PH_m[36:80] + PropN_PH_f[36:80])/2
  mCx_PNH <- (PropN_PNH_m[36:80] + PropN_PNH_f[36:80])/2
  mCx_NPH <- (PropN_NPH_m[36:80] + PropN_NPH_f[36:80])/2
  mCx_NPNH <- (PropN_NPNH_m[36:80] + PropN_NPNH_f[36:80])/2
...
# The remaining life expectancy sex gap at the age 60
# has similar outlook as at age 25. However, the LE
# sex gap has shrunk to 2.9 with composition component 
# rising to -0.74, a 0.07 gain. Mortality component decrease
# is warranted since at older ages male and female mortality 
# differences slightly narrowed. 